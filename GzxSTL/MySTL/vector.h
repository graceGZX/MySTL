...
public:
  ...
  vector& operator=(std::initializer_list<value_type> ilist)   //重载 =
  {
...
get_new_cap(size_type add_size)
{
  ...
  THROW_LENGTH_ERROR_IF(old_size > max_size() - add_size,    //判断需要增加的大小是否超出界限
...
// 重新分配空间并在 pos 处插入元素                 //forward和copy
template <class T>
